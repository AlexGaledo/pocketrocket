; Tauri's English strings with deleteAppData reworded. Saved WITHOUT a byte-order mark: the bundler
; prepends its own, and a second one makes makensis reject line 1. Language files are copied, not
; rendered, so use NSIS defines like ${PRODUCTNAME} rather than {{handlebars}} placeholders.
...
LangString deleteAppData ${LANG_ENGLISH} "Also delete all bots, conversations and settings"
