...
```bash
# once, on the VPS: install Node 22.13+, pnpm, and your chosen provider's CLI, then log in
...
The service (`/etc/systemd/system/pocketrocket.service`) binds `127.0.0.1:7788` only. **Never expose the port directly** — reach it over an SSH tunnel or Tailscale; the hub has no auth beyond loopback (and a bearer token in desktop mode). Data lives in `~/pocketrocket/data/` on the VPS. Logs: `journalctl -u pocketrocket -f`.

### Screen: a persistent desktop the bots and you share

`deploy/setup-vps.sh` (run by `scripts/deploy.sh`) installs Xvfb + XFCE + x11vnc + noVNC + xdotool/scrot + a Playwright Chromium, and starts `pocketrocket-screen.service`.

What persists (all under `data/`, backed up daily to `/root/pocketrocket-backups`, 7 kept):
...
Using it: the hub proxies noVNC at `/screen/`, so the same SSH tunnel is enough — open the **Screen** tab, click inside to use the desktop, log into accounts in Chrome once. A **Browser** tool (Playwright MCP over CDP) gives bots fast, precise control of that logged-in Chrome. A **Desktop** tool gives bots full computer use (screenshot, click, type, key, scroll, launch apps) for anything Browser can't reach — slower and costlier (~1.2k tokens per screenshot), so bots are told to prefer shell/file/Browser tools first. Neither tool goes through approval cards, so only give them to bots you trust with the logged-in sessions on that machine.

Ops: `systemctl status pocketrocket-screen`, `journalctl -u pocketrocket-screen -f`. Restore a backup: stop both services, untar into `/root/pocketrocket`, start them again.
